...
- **`reconocimiento.html` existe y el prompt no lo menciona.** Es una página real (el mismo premio VISA · La Nación que también cubre `prensa.html`), ya migrada al sistema visual nuevo — solo le faltaban metadatos, que ya quedaron al día. (Corrección: en una nota anterior de este documento decía que esta página seguía con el diseño viejo — estaba mal, la que realmente sigue vieja es `terminos-y-condiciones.html`, ver auditoría de coherencia.)
